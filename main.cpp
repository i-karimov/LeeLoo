...
public:
    ...
    Token get() {
        ...
        switch (ch) {
            ...
            case '(':
                return Token(Token::l_round_b);
            case ')':
                return Token(Token::r_round_b);
            case ';':
...
double primary() {
    ...
    switch (t.kind) {
        ...
        case Token::minus:
            return -primary();
        case Token::l_round_b: {
            double exp = expression();
            Token rb = ts.get();
            if (rb.kind != Token::r_round_b) throw Error("Primary error: closing round bracket expected.");
            return exp;
        }
        default:
